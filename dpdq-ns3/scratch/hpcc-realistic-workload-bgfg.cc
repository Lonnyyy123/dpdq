...
uint32_t creditbouncer_grant_granularity_bytes = 1000;
double creditbouncer_ai_step_bytes = 1000.0;
double creditbouncer_md_factor = 0.5;
uint32_t creditbouncer_grant_interval_ns = 1000;
...
uint32_t creditbouncer_dedicated_credit_queue_bytes = 64 * 1024;
...
void qp_finish(FILE* fout, Ptr<RdmaQueuePair> q){
	...
	uint64_t base_rtt = pairRtt[snode][dnode], b = pairBw[snode][dnode];
...
int main(int argc, char *argv[])
{
...
#endif
	{
		...
		while (!conf.eof())
		{
            ...
            // Added From Here
				...
				std::cerr << "CREDITBOUNCER_DEDICATED_CREDIT_QUEUE_BYTES\t" << creditbouncer_dedicated_credit_queue_bytes << "\n";
    ...
    const double LINK_RATE_IN_GBPS = 50.;
	...
	std::cerr << "Link Rate : " << LINK_RATE_IN_GBPS << "Gbps, Load : " << load << ", AvgFlowSize: " << avg_flow_size << ", workload: " << hpcc_workload << std::endl;
	...
	double fg_lambda = (double) LINK_RATE_IN_GBPS * 1e9 * load * foreground_flow_ratio / (8. * FOREGROUND_INCAST_FLOW_PER_HOST * incast_flow_size * ((double)MTU_SIZE)/MSS_SIZE) / oversubscription_ratio;
    ...
    unsigned int remaining_flows = num_bg_flows;
	...
	for (uint32_t i = 0; i < remaining_flows; i++) {
		uint32_t src, dst, maxPacketCount, port;
		...
		src = send_host+switch_num;
		dst = recv_host+switch_num;
	...
	last_background_flow_time = current_time;
	std::cerr << "Background flow ends at " << last_background_flow_time << ", foreground flow interval =" << large_foreground_interval.GetSeconds() << std::endl;
	current_time = app_start_time+0.01+large_foreground_interval.GetSeconds();

    if(current_time > 0 && foreground_flow_ratio > 0) {
		for (int i=0; i< num_bg_flows; i++) {
			if(last_background_flow_time < current_time)
				break;
			...
			for(unsigned int j=0; j< node_num - switch_num ; j++) {
				...
				for(unsigned int k =0; k < FOREGROUND_INCAST_FLOW_PER_HOST; k++) {
					uint32_t src, dst, maxPacketCount, port;
					// while (used_port[port = int(uniform_rv->GetInteger(0, 40000))])//.GetValue() * 40000)])
					// 	continue;
					// used_port[port] = true;
					
					// port = port_num++;
					port = port_per_host[recv_host]++;
					uint32_t flow_size = incast_flow_size;
					maxPacketCount = (flow_size + MSS_SIZE - 1)/ MSS_SIZE;
					src = switch_num+j;
					dst = switch_num+recv_host;
                    ...
                    port = portNumder[src]++; // get a new port number 
                    RdmaClientHelper clientHelper(pg, serverAddress[src], serverAddress[dst], port, dport, flow_size, has_win?(global_t==1?maxBdp:pairBdp[n.Get(src)][n.Get(dst)]):0, global_t==1?maxRtt:pairRtt[n.Get(src)][n.Get(dst)]);
                    // clientHelper.SetAttribute("MaxPackets", UintegerValue(maxPacketCount));
                    // clientHelper.SetAttribute("Interval", TimeValue(interPacketInterval));
                    // clientHelper.SetAttribute("PacketSize", UintegerValue(packetSize+12)); // Important! Include SeqTsHeader
					clientHelper.SetAttribute("StatFlowID", IntegerValue(flow_id));
                    ...
                    appCon.Start(Seconds(current_time));
                    appCon.Stop(Seconds(app_stop_time));
					// std::cerr << "Flow " << flow_id << " : Total length : " << flow_size << " at " << current_time << std::endl;
					flow_id++;
				}
			}
