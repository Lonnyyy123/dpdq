...
namespace ns3 {
	TypeId SwitchMmu::GetTypeId(void){
		static TypeId tid = TypeId("ns3::SwitchMmu")
			...
			.AddAttribute("DynamicThreshold",
						  "Broadcom Egress alpha",
						  BooleanValue(true),
						  MakeBooleanAccessor(&SwitchMmu::SetDynamicThreshold, &SwitchMmu::GetDynamicThreshold),
						  MakeBooleanChecker())
			.AddAttribute("MaxTotalBufferPerPort",
						  "Maximum buffer size of MMU per port in bytes (12-port switch: 12 * 200kB = 2.4MB)",
						  UintegerValue(200*1000),
						  MakeUintegerAccessor(&SwitchMmu::SetMaxBufferBytesPerPort, &SwitchMmu::GetMaxBufferBytesPerPort),
			...
			.AddAttribute("ActivePortCnt",
						  ...
						  MakeUintegerChecker<uint32_t>())
			.AddAttribute("PGHeadroomLimit",
						  "Headroom Limit per PG",
			...
			.AddAttribute("TltEnable",
						"Enable TLT",
						BooleanValue(false),
						MakeBooleanAccessor(&SwitchMmu::m_tlt),
						MakeBooleanChecker())
			.AddAttribute("TltUipDropRate",
						"TLT Drop Rate of Unimportant packet. Use only for debugging",
						DoubleValue(0.0),
						MakeDoubleAccessor(&SwitchMmu::m_tlt_uimp_drop_rate),
						MakeDoubleChecker<double>())
			.AddAttribute("MaxBytesTltUip",
						  "The maximum number of bytes unimportant packets (TLT)",
						  DoubleValue(0),
						  MakeDoubleAccessor(&SwitchMmu::m_maxBytes_TltUip),
						  MakeDoubleChecker<double>())
		;
		return tid;
	}
	SwitchMmu::SwitchMmu(void){
		// Default buffer size: 200kB per active port
		// 12-port switch: 12 * 200kB = 2.4MB
		// 32-port switch: 32 * 200kB = 6.4MB
	...
	void SwitchMmu::InitSwitch(void){
		...
		// m_pg_hdrm_limit = 103000; //2*10us*40Gbps+2*1.5kB //106 * MTU; //ingress pg headroom // set dynamically
		m_port_max_pkt_size = 100 * MTU; //ingress global headroom
		uint32_t total_m_pg_hdrm_limit = 0;
		for (int i = 0; i < m_activePortCnt; i++)
			total_m_pg_hdrm_limit += m_pg_hdrm_limit[i];
		m_buffer_cell_limit_sp = m_maxBufferBytes - total_m_pg_hdrm_limit - (m_activePortCnt)*std::max(qCnt * m_pg_min_cell, m_port_min_cell); //12000 * MTU; //ingress sp buffer threshold
		//still needs reset limits..
		m_port_min_cell_off = 4700 * MTU;
		m_pg_shared_limit_cell_off = m_pg_shared_limit_cell - 2 * MTU;

		//egress params
		m_op_buffer_shared_limit_cell = m_maxBufferBytes - (m_activePortCnt)*std::max(qCnt*m_pg_min_cell, m_port_min_cell);//m_maxBufferBytes; //per egress sp limit
	...
	bool SwitchMmu::CheckIngressAdmission(uint32_t port, uint32_t qIndex, uint32_t psize){
		...
		if (m_usedIngressPGBytes[port][qIndex] + psize > m_pg_min_cell && m_usedIngressPortBytes[port] + psize > m_port_min_cell) // exceed guaranteed, use share buffer
		{
			if (m_usedIngressSPBytes[GetIngressSP(port, qIndex)] > m_buffer_cell_limit_sp) // check if headroom is already being used
			{
				...
				{
					if(m_PFCenabled)
						std::cerr << "WARNING: Drop because ingress headroom full:" << m_usedIngressPGHeadroomBytes[port][qIndex] << "\t" << m_pg_hdrm_limit << " tlt uip " << m_maxBytes_TltUip << "\n"; 
					return false;
				}
			}
		}
	...
	bool SwitchMmu::CheckEgressAdmission(uint32_t port, uint32_t qIndex, uint32_t psize){
		...
		if ((double)m_usedEgressQSharedBytes[port][qIndex] + psize  > m_pg_shared_alpha_cell_egress*((double)m_op_buffer_shared_limit_cell - m_usedEgressSPBytes[GetEgressSP(port, qIndex)])) {
			// std::cerr << "WARNING: Drop because egress DT threshold exceed for port: " << port << " queue " << qIndex << ". Natural if not using PFC";
			threshold = false;
	...
	void SwitchMmu::printQueueStat(std::ostream & os, uint32_t port) {
		os << "EG_PORT_" << port << " UIP=" << m_bytesEgressTltUip[port] << ", PORT=" << m_usedEgressPortBytes[port] << std::endl << std::flush;
	}
	...
	void SwitchMmu::UpdateEgressAdmission(uint32_t port, uint32_t qIndex, uint32_t psize){
		if (m_usedEgressQMinBytes[port][qIndex] + psize < m_q_min_cell)    //guaranteed
		{
			...
			#if EGRESS_STAT
			if(printQueueLen)
					fprintf(stderr, "%.8lf\tEGRESS_QUEUE\t%p\t%u\t%u\t%u\n", Simulator::Now().GetSeconds(), this, port, 
	...
	bool SwitchMmu::ShouldSendCN(uint32_t ifindex, uint32_t qIndex) {
		...
		else if (m_usedEgressQSharedBytes[ifindex][qIndex] > kmin[ifindex] && kmin[ifindex] != kmax[ifindex])
		{
			double p = 1.0 * (m_usedEgressQSharedBytes[ifindex][qIndex] - kmin[ifindex]) / (kmax[ifindex] - kmin[ifindex]) * pmax[ifindex];
			...
			uint32_t pg_qcn_threshold)
	{
		...
		for (int i = 0; i < pCnt; i++)
			m_pg_hdrm_limit[i] = pg_hdrm_limit;
...
uint32_t
	SwitchMmu::GetUsedBufferTotal()
{
	return m_usedTotalBytes;
}

uint32_t
	SwitchMmu::GetEgressDataQueueBytes(uint32_t port) const
{
	uint32_t bytes = 0;
	for (uint32_t q = 1; q < qCnt; ++q)
		bytes += m_usedEgressQMinBytes[port][q] + m_usedEgressQSharedBytes[port][q];
	return bytes;
}

void
	SwitchMmu::SetDynamicThreshold(bool v)
	{
		m_dynamicth = v;
		InitSwitch();
		return;
	...
	void SwitchMmu::SetPause(uint32_t port, uint32_t qIndex, uint32_t pause_time){
		...
		resumeEvt[port][qIndex] = Simulator::Schedule(MicroSeconds(pause_time), &SwitchMmu::SetResume, this, port, qIndex);
	}
	...
	bool SwitchMmu::CheckIngressTLT(uint32_t port, uint32_t qIndex, uint32_t psize) {
		return true;
	}

	bool SwitchMmu::CheckEgressTLT(uint32_t port, uint32_t qIndex, uint32_t psize) {
		// if m_maxBytes_TltUip is 0, regard TLT is disabled
		if (m_tlt && m_maxBytes_TltUip && m_bytesEgressTltUip[port] + psize >= m_maxBytes_TltUip) {
			// std::cerr << "Warning: Dropping unimportant packet by buffer full. m_bytesEgressTltUip=" << m_bytesEgressTltUip[port] << std::endl;
			return false;
		}
		if (m_tlt && m_tlt_uimp_drop_rate > 0) {
			if (m_uniform_random_var.GetValue(0, 1) < m_tlt_uimp_drop_rate) {
				std::cerr << "Warning: Dropping unimportant packet randomly. Use only on debugging." << std::endl;
				return false;
			}
		}
		return true;
	...
	void SwitchMmu::UpdateIngressTLT(uint32_t port, uint32_t qIndex, uint32_t psize) {
		m_bytesIngressTltUip[port] += psize;
		m_totaluipbytes += psize;
		return;
	}
	void SwitchMmu::UpdateEgressTLT(uint32_t port, uint32_t qIndex, uint32_t psize) {
		m_bytesEgressTltUip[port] += psize;

		return;
	}
	void SwitchMmu::RemoveFromIngressTLT(uint32_t port, uint32_t qIndex, uint32_t psize) {
		m_bytesIngressTltUip[port] -= psize;
		m_totaluipbytes -= psize;
	}

	void SwitchMmu::RemoveFromEgressTLT(uint32_t port, uint32_t qIndex, uint32_t psize) {
		m_bytesEgressTltUip[port] -= psize;
	}
