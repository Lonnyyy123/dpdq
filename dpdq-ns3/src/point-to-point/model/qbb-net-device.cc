...
namespace ns3 {
	...
	Ptr<Packet> RdmaEgressQueue::DequeueQindex(int qIndex){
		...
		if (qIndex >= 0){ // qp
			std::cerr << "Dequeue from qIndex " << m_qpGrp->Get(qIndex)->m_flow_id << std::endl;
	...
	int RdmaEgressQueue::GetNextQindex(bool paused[]){
		...
		if (!m_rdmaGetNextCreditBouncerQindex.IsNull()) {
			int cbQindex = m_rdmaGetNextCreditBouncerQindex(m_qpGrp, paused, m_rrlast, m_mtu);
			if (cbQindex >= 0)
				return cbQindex;
		}
		...
		for (qIndex = 1; qIndex <= fcount; qIndex++){
			...
			bool cond2 = (!qp->cb.m_enabled && qp->GetBytesLeft() > 0 && cond_window_allowed) || cond_tlt;
		...
		QbbNetDevice::GetTypeId(void)
	{
		static TypeId tid = TypeId("ns3::QbbNetDevice")
			...
			.AddTraceSource ("QbbPfcState", "PFC state transition on this device. args: ifIndex, qIndex, type (0=resume,1=pause)",
					MakeTraceSourceAccessor (&QbbNetDevice::m_tracePfcState))
			;
		...
		QbbNetDevice::DequeueAndTransmit(void)
	{
		...
		if (m_node->GetNodeType() == 0){
			...
			if (qIndex != -1024){
				...
				p = m_rdmaEQ->DequeueQindex(qIndex);
				if(p == 0){
					NS_LOG_ERROR("DequeueQindex returns null for qIndex " << qIndex);
					return;
				}
				// transmit
		...
		QbbNetDevice::Receive(Ptr<Packet> packet)
	{
		...
		packet->PeekHeader(ch);
			if (ch.l3Prot == 0xFE){ // PFC
				...
				if (ch.pfc.time > 0){
					m_tracePfc(1);
					m_tracePfcState(m_ifIndex, qIndex, 1);
					...
					m_tracePfc(0);
					m_tracePfcState(m_ifIndex, qIndex, 0);
					Simulator::Cancel(m_resumeEvt[qIndex]);
					Resume(qIndex);
				}
