...
set send_events_file "${results_path}/send_events.csv"
...
set send_events_init [open $send_events_file "w"]
close $send_events_init
...
# When num_aggr == 0, connect tor nodes directly (true dumbbell)
if {$num_aggr == 0} {
    puts -nonewline $parmtr_file "aggr_addr "
    for {set j 0} {$j < $num_tors} {incr j} {
        for {set k [expr {$j + 1}]} {$k < $num_tors} {incr k} {
            $ns simplex-link $tor_node($j) $tor_node($k) ${core_link_speed_gbps}Gb ${core_link_latency_ms}ms $core_sw_q_type
            $ns simplex-link $tor_node($k) $tor_node($j) ${core_link_speed_gbps}Gb ${core_link_latency_ms}ms $core_sw_q_type

            set link [$ns link $tor_node($j) $tor_node($k)]
            set myif [new NetworkInterface]
            set head [$link head]
            $myif target [$head target]
            $head target $myif
            $myif label $iflabel_cnt
            set q [[$ns link $tor_node($j) $tor_node($k)] queue]
            set qcls [$q info class]
            if {$qcls == "Queue/DropTail" || $qcls == "Queue/RED" || $qcls == "Queue/DPDQQueue"} {
                $q drop-log-file $dpdq_drop_events_file
                $q set-owner-id [$tor_node($j) node-addr]
                $q set-peer-id [$tor_node($k) node-addr]
                $q set-iface-label $iflabel_cnt
            }
            $classifier_tor($j) set-egress-queue $iflabel_cnt $q
            $classifier_tor($j) set-peer-id $iflabel_cnt [$tor_node($k) node-addr]
            set pqlenInt_ [new Integrator]
            $classifier_tor($j) set-pqlen-integrator $iflabel_cnt $pqlenInt_

            set link [$ns link $tor_node($k) $tor_node($j)]
            set myif [new NetworkInterface]
            set head [$link head]
            $myif target [$head target]
            $head target $myif
            $myif label $iflabel_cnt
            set q [[$ns link $tor_node($k) $tor_node($j)] queue]
            set qcls [$q info class]
            if {$qcls == "Queue/DropTail" || $qcls == "Queue/RED" || $qcls == "Queue/DPDQQueue"} {
                $q drop-log-file $dpdq_drop_events_file
                $q set-owner-id [$tor_node($k) node-addr]
                $q set-peer-id [$tor_node($j) node-addr]
                $q set-iface-label $iflabel_cnt
            }
            $classifier_tor($k) set-egress-queue $iflabel_cnt $q
            $classifier_tor($k) set-peer-id $iflabel_cnt [$tor_node($j) node-addr]
            set pqlenInt_ [new Integrator]
            $classifier_tor($k) set-pqlen-integrator $iflabel_cnt $pqlenInt_

            incr iflabel_cnt

            puts -nonewline $parmtr_file "[$tor_node($j) node-addr] [$tor_node($k) node-addr] "
            # Attach tor->tor link to node for shared switch buffer mode
            $tor_node($j) attach-egress-queue [[$ns link $tor_node($j) $tor_node($k)] queue]
            # Attach tor->tor link to node for shared switch buffer mode
            $tor_node($k) attach-egress-queue [[$ns link $tor_node($k) $tor_node($j)] queue]
        }
    }
    puts $parmtr_file ""
}
        ...
        foreach host_in $r2p2_hosts {
            ...
            if {$host != $host_in} {
                ...
                if {($is_client && $is_server_in) || ($is_server && $is_client_in)} {
                    ...
                    if {$transport_protocol == "r2p2"} {
                        set this_agent [dict get $agent_host_host "${host}_$host_in"]
                        $this_agent set-send-log-file $send_events_file
                    }
...
# Detect benchmark mode: num_aggr == 0 with paired hosts
set is_benchmark [expr {$num_aggr == 0 && $num_tors == 2}]
if {$is_benchmark} {
    puts "Benchmark mode: dumbbell with paired hosts. s0->r0 uses w4, others burst at start_at+0.03s"
}
...
for {set app 0} {$app < $num_client_apps} {incr app} {
    ...
    if {$is_benchmark && $app > 0} {
        # Burst clients: no periodic requests, will trigger single request later
        $clientApplication($app) set request_interval_sec_ 0
        $clientApplication($app) set request_size_B_ $mean_req_size_B
        $clientApplication($app) set-req-dstr fixed [expr $app + $app * 133 + 1]
        $clientApplication($app) set-req-sz-dstr fixed [expr $app + $app * 133 + 1]
    } else {
...
for {set app 0} {$app < $num_server_apps} {incr app} {
    ...
    if {$is_benchmark} {
        # All servers use fixed 4-byte response in benchmark mode
        $serverApplication($app) set response_size_B_ 4
        $serverApplication($app) set-resp-sz-dstr fixed [expr $app + $app * 150 + 2]
    } else {
...
for {set j 0} {$j < $num_client_apps} {incr j} {
    ...
    if {$is_benchmark && $j > 0} {
        # Burst clients: start app (to init tracing etc.) then trigger single request at start_at + 0.02
        set burst_size [expr {[info exists burst_req_size_B] ? $burst_req_size_B : 204800}]
        $ns at $start_at "$clientApplication($j) start"
        $ns at [expr $start_at + 0.02] "$clientApplication($j) send-single-request $burst_size"
    } else {
        $ns at $start_at "$clientApplication($j) start"
    }
